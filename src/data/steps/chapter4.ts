...
export async function DynamicProfilePage() {
  // キャッシュなしの fetch -> 自動的に SSR 挙動へパンプアップ！
...
keyPoints: [
  "SSG: ビルド時にHTML作成。表示速度が最速でCDNキャッシュに最適",
  "SSR: リクエスト毎にサーバー生成。常に最新データが必要なページ向け",
...
explanation: "App Routerの基本思想は『原則サーバー側で描画する』です。重いライブラリやDB直接アクセスのコードをサーバー側に閉じ込め、JSバンドルサイズをゼロに近づけます。onClickやuseStateなどのインタラクティブな処理が必要な末端のコンポーネントだけを 'use client' で切り離すのが最強のフォームです。",
...
explanation: "ファイルシステムベースルーティングにおいて、URLパラメーター（[id]）やクエリ文字列（?page=1）を TypeScript で型安全に受け取るテクニックです。",
...
explanation: "従来のように REST API や fetch を自作・呼び出す必要なく、サーバー側で実行される関数をクライアントのフォームやイベントから直接呼び出せます。",
